...
type Esito = { ok: boolean; warning?: string; perAltro: boolean };

/**
 * Form della richiesta: viene montato solo mentre la modale è aperta, così ogni apertura riparte
 * da uno stato pulito (date, errori del tentativo precedente, esito dell'action).
 */
function AbsenceRequestForm({
  ...
  const [state, formAction] = useActionState<TeamActionState, FormData>(async (prev, fd) => {
    ...
    if (r.ok) {
      ...
      onDone({ ok: true, warning: r.warning, perAltro: isAdmin && !!userId && userId !== currentUserId });
    }
    return r;
  }, {});
      ...
      {esito && (
        <div className="fixed left-1/2 top-4 z-50 w-[calc(100%-1.5rem)] max-w-md -translate-x-1/2 shadow-lg" role="status">
          <Alert kind={esito.warning ? "warning" : "success"} title={esito.perAltro ? "Assenza registrata e approvata." : "Richiesta inviata agli amministratori."}>
            ...
            {!esito.perAltro && <p>La trovi nella sezione «In attesa di approvazione» qui sotto.</p>}
